...
import 'package:gui/widgets/rom_card.dart';
...
void main() {
  group('Error Dialog & RomCard Tests', () {
    testWidgets(
      'showOpenROMError displays dialog with title, message and action',
      (tester) async {
        ...
        expect(
          find.text('openrom-core is missing from the app folder.'),
          findsOneWidget,
        );
        expect(
          find.text('Please re-download the full ZIP from GitHub.'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'RomCard displays failed state with error icon and responds to tap',
      (tester) async {
        final rom = RomFile(
        ...
        await tester.pumpWidget(
          _wrapWithLocalizations(
            RomCard(job: job, theme: theme, onDelete: () {}),
          ),
        );
