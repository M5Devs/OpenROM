...
///
/// Dart has no CRC-32 in its standard library, so we implement a small
/// table-based, streaming version that can be fed chunk by chunk.
class Crc32 {
  ...
  /// Feeds [bytes] (optionally a `[start, end)` slice) into the running CRC.
  ...
  /// The final CRC-32 value as an unsigned 32-bit integer.
  int get value => (_crc ^ 0xFFFFFFFF) & 0xFFFFFFFF;
}

/// Computes the CRC-32 of [bytes] (optionally a `[start, end)` slice).
int crc32Bytes(List<int> bytes, [int start = 0, int? end]) {
  final crc = Crc32();
  crc.update(bytes, start, end);
  return crc.value;
}

/// Computes the CRC-32 of an entire file by streaming it in
/// [patchCopyBufferSize] chunks, avoiding loading large ROMs fully into memory.
Future<int> crc32OfFile(File file) async {
  final crc = Crc32();
...
/// CRC-16/CCITT-FALSE implemented via a lookup table. Ported from UniPatcher's
/// `utils/Crc16.kt`; used by the APS (GBA) format to validate 64 KB chunks.
class Crc16 {
  ...
  static int calculate(List<int> data) {
    ...
    for (final byte in data) {
      crc = ((crc << 8) ^ _table[((crc >> 8) ^ (byte & 0xFF)) & 0xFF]) & 0xFFFF;
    }
    return crc;
  }
}
