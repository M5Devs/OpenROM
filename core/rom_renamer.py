...
def import_dat(dat_path: str) -> dict:
    ...
    header_info = _read_dat_header(dat_path)
    dat_name = header_info["name"]
    dat_url  = header_info.get("url", "")
    source   = header_info["source"]
    game_count = header_info["game_count"]
...
def _read_dat_header(dat_path: str) -> dict:
    """Read header info and game count from a DAT file using iterparse for memory efficiency."""
    dat_name = ""
    dat_url = ""
    game_count = 0
    header_found = False

    for event, elem in ET.iterparse(dat_path, events=("start", "end")):
        if event == "end" and elem.tag == "header":
            header_found = True
            name_el = elem.find("name")
            url_el  = elem.find("url")
            dat_name = name_el.text.strip() if name_el is not None and name_el.text else os.path.basename(dat_path)
            dat_url  = url_el.text.strip()  if url_el  is not None and url_el.text  else ""
            elem.clear()
        elif event == "end" and elem.tag == "game":
            game_count += 1
            elem.clear()

    if not header_found and not dat_name:
    ...
    return {"name": dat_name, "source": source, "game_count": game_count, "url": dat_url}
...
def load_dat_index(dat_path: str) -> dict[str, dict]:
    """
    Load a DAT file and return a dict indexed by CRC32 (lowercase hex) using streaming iterparse.
    ...
    for event, elem in ET.iterparse(dat_path, events=("end",)):
        if elem.tag == "game":
            ...
            desc      = desc_el.text.strip() if desc_el is not None and desc_el.text else game_name
